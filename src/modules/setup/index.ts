...
export const loadEtc = async (given_setup: Setup<any>) => {
...
export const processCliArgs = async (given_setup: Setup<any>) => {
				...
				if(process.argv[++i] != null) {
					...
					switch(nPath.extname(file_path)) {
						...
						case ".yml":
							...
							break;
						case ".json":
							file_data = JSON.parse(<string>await File.read(file_path));
							break;
					}
...
export const checkDirectories = async (given_setup: Setup<any>) => {
