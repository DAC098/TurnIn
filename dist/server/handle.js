...
const handle = async (req, res) => {
    ...
    req.url_parsed = new URL(req.url, `https://${req.headers['host']}/`);
    try {
        logger.info(`${req.method} ${req.url} HTTP/${req.httpVersion}`);
        let result = await router.run(req, res);
        if (!result.found_path) {
            logger.info(`${req.method} ${req.url} HTTP/${req.httpVersion}: not found`);
            await res.endJSON(404, {
                'message': 'not found'
            });
        }
        else if (result.found_path && !result.valid_method) {
            logger.info(`${req.method} ${req.url} HTTP/${req.httpVersion}: invalid method`);
